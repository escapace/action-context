...
describe('getPullRequest', () => {
  ...
  it('returns PR data for a non-draft bot PR', async () => {
    ...
    const result = await getPullRequest(octokit, 95)
  ...
  it('throws descriptive error on 403', async () => {
    ...
    await expect(getPullRequest(octokit, 95)).rejects.toThrow(
  ...
  it('rethrows non-403 errors', async () => {
    ...
    await expect(getPullRequest(octokit, 95)).rejects.toThrow('Internal Server Error')
  })
