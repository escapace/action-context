...
describe('fetchReviewData', () => {
  ...
  it('throws descriptive error on 403', async () => {
    ...
    await expect(fetchReviewData(octokit as never, 95)).rejects.toThrow(
  ...
  it('rethrows non-403/non-forbidden errors', async () => {
    ...
    await expect(fetchReviewData(octokit as never, 95)).rejects.toThrow('Something went wrong')
  })
  ...
  it('calls GraphQL with correct variables', async () => {
    ...
    const result = await fetchReviewData(octokit as never, 95)
