...
| Explicit pull request mode | `workflow_call`, `workflow_dispatch`, `schedule` | `pr`              | `pr-number` required; `pr-head-ref`/`pr-head-sha` optional guards | populated from explicit PR number and current API state; invalid PR inputs fail fast (`INPUT_INVALID`) |
...
- uses: escapace/action-context@0.13.1
  id: context
```
...
When `pr-head-ref` or `pr-head-sha` is provided and does not match current API data, the action fails with `INPUT_INVALID`.

Providing `pr-number`, `pr-head-ref`, or `pr-head-sha` with `context-source=event` (the default) fails with `INPUT_INVALID`.
...
| `pr-head-ref`    | no       | _(empty)_      | Optional guard for `context-source=pr`; mismatch fails with `INPUT_INVALID`. Rejected in event mode. |
| `pr-head-sha`    | no       | _(empty)_      | Optional guard for `context-source=pr`; mismatch fails with `INPUT_INVALID`. Rejected in event mode. |
| `pr-number`      | no       | _(empty)_      | PR number used when `context-source=pr`; must be a positive integer. Rejected in event mode.         |
...
In explicit pull request mode, invalid PR input guards (`pr-number`, `pr-head-ref`, `pr-head-sha`) fail fast with `INPUT_INVALID` before output collection. Providing these inputs in event mode also fails with `INPUT_INVALID`.
