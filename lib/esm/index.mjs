...
var require_register = __commonJS({
  "node_modules/.pnpm/before-after-hook@2.2.2/node_modules/before-after-hook/lib/register.js"(exports, module) {
...
var require_add = __commonJS({
  "node_modules/.pnpm/before-after-hook@2.2.2/node_modules/before-after-hook/lib/add.js"(exports, module) {
...
var require_remove = __commonJS({
  "node_modules/.pnpm/before-after-hook@2.2.2/node_modules/before-after-hook/lib/remove.js"(exports, module) {
...
var require_before_after_hook = __commonJS({
  "node_modules/.pnpm/before-after-hook@2.2.2/node_modules/before-after-hook/index.js"(exports, module) {
...
var require_lib3 = __commonJS({
  "node_modules/.pnpm/node-fetch@2.6.7/node_modules/node-fetch/lib/index.js"(exports, module) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
var getEnv = ({ env: envOption, extendEnv, preferLocal, localDir, execPath }) => {
  const env = extendEnv ? { ...process3.env, ...envOption } : envOption;
  if (preferLocal) {
...
var handleArguments = (file, args, options = {}) => {
  ...
  options = {
    ...
    extendEnv: true,
    preferLocal: false,
    localDir: options.cwd || process3.cwd(),
  ...
  if (process3.platform === "win32" && path2.basename(file, ".exe") === "cmd") {
    args.unshift("/q");
  }
...
var exec = async (cmd, args) => {
  const res = await execa(cmd, args);
  return res.stdout.trim();
};
var SEMVER_OPTIONS = { loose: false, includePrerelease: true };
var COMMITISH = github.context.sha.slice(0, 7);
...
var assertRepoLatestCommit = async () => {
  if (REF_TYPE === "branch") {
    assert.equal(await exec("git", ["rev-parse", "--verify", REF_NAME]), github.context.sha);
  }
};
async function getLastGitVersion() {
  const list = (await exec("git", ["--no-pager", "tag", "-l", "--sort=creatordate"])).split("\n").map((value) => import_semver.default.clean(value, SEMVER_OPTIONS)).filter((value) => isString_default(value));
  const listSorted = [...list].sort((a, b) => import_semver.default.compareBuild(a, b, SEMVER_OPTIONS)).reverse();
...
var toSemver = (props) => {
  ...
  const string = `${major}.${minor}.${patch}${prerelease.length === 0 ? "" : `-${prerelease.join(".")}`}${build.length === 0 ? "" : `+${build.join(".")}`}`;
  const version = import_semver.default.parse(string, SEMVER_OPTIONS);
  if (version === null) {
...
var getVersion = async () => {
  if (REF_TYPE === "tag") {
    const version = import_semver.default.parse(import_semver.default.clean(REF_NAME, SEMVER_OPTIONS), SEMVER_OPTIONS);
    if (version === null) {
    ...
    if (lastGitVersion === void 0) {
      return import_semver.default.parse(`0.1.0-${REF_NAME}+${COMMITISH}`, SEMVER_OPTIONS);
    } else {
      const { major, minor, patch, prerelease, build } = import_semver.default.parse(lastGitVersion, SEMVER_OPTIONS);
      return toSemver({
        major,
        minor: minor + 1,
        patch,
        prerelease: [
          ...prerelease,
          last_default(prerelease) === REF_NAME ? void 0 : REF_NAME
        ].filter((value) => isString_default(value)),
        build: [
          ...build,
          last_default(build) === COMMITISH ? void 0 : COMMITISH
        ].filter((value) => isString_default(value))
      });
    }
  }
...
var run = async () => {
  ...
  const environment = isTag ? isPrerelese ? "staging" : "production" : "testing";
  core.setOutput("version", version);
  core.setOutput("environment", environment);
  core.setOutput("commitish", COMMITISH);
};
function handleError(err) {
  const message = isError_default(err) ? err.message : isString_default(err) ? err : "Unknown Error";
