...
__export(src_exports, {
  ...
  getLastGitVersion: () => getLastGitVersion
});
...
async function getLastGitVersion() {
  ...
  const listSorted = [...list].sort((a, b) => import_semver.default.compareBuild(a, b, SEMVER_OPTIONS));
  if (listSorted[0] !== list[0]) {
    core.debug(`Different first entry in:
  ...
  if (REF_TYPE === "tag") {
    ...
    if (lastGitVersion === void 0) {
      ...
      return toSemver({
        ...
        ].filter((value) => isString_default(value) || isNumber_default(value)),
        build: [
          ...build,
