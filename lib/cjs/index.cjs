...
__export(src_exports, {
  assertRepoNotShallow: () => assertRepoNotShallow,
  getLastGitTag: () => getLastGitTag2
});
...
// node_modules/.pnpm/changelogen@0.1.1/node_modules/changelogen/dist/chunks/markdown.mjs
...
async function execCommand(cmd, args) {
  ...
  const res = await execa2(cmd, args);
  return res.stdout;
}
...
var DEFAULT_INCREMENT = "minor";
var assertRepoNotShallow = async () => import_node_assert.default.notEqual(await exec("git", ["rev-parse", "--is-shallow-repository"]), "true");
var assertRepoLatestCommit = async () => {
  if (REF_TYPE === "branch") {
    import_node_assert.default.equal(await exec("git", ["rev-parse", "--verify", REF_NAME]), github.context.sha);
  }
};
var asserPreReleaseIdentifier = () => {
  if (REF_TYPE === "branch") {
    import_node_assert.default.ok(/^[a-zA-Z0-9-]+$/.test(REF_NAME), "Branch name does not pass /^[a-zA-Z0-9-]+$/.");
  }
};
async function getLastGitTag2() {
  const list = (await exec("git", ["--no-pager", "tag", "-l", "--sort=creatordate"])).split("\n").filter((value) => import_semver.default.clean(value, SEMVER_OPTIONS) !== null);
  const listSorted = [...list].sort((a, b) => import_semver.default.compareBuild(import_semver.default.clean(a), import_semver.default.clean(b), SEMVER_OPTIONS));
  core.debug(`getLastGitVersion():
 ${JSON.stringify([list, listSorted])}`);
  if (last_default(listSorted) !== last_default(list)) {
...
var bump = async (lastGitTag, value) => {
  ...
  }).filter((value2) => isString_default(value2)).reduce((prev, next) => {
    prev[next] = true;
    return prev;
...
var getVersion = async () => {
  if (REF_TYPE === "tag") {
    ...
    const lastGitTag = await getLastGitTag2();
    if (lastGitTag === void 0) {
      ...
      const { major, minor, patch } = import_semver.default.parse(import_semver.default.clean(lastGitTag), SEMVER_OPTIONS);
      return toSemver({
