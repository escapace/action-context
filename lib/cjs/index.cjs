...
var getBranch = () => {
  ...
  if (!isString_default(value)) {
    import_node_assert.default.ok(`Expected ${ref} to match '/refs\\/heads\\/(?<value>[^/]+)/'`);
  }
...
var assertRepoLatestCommit = async (branch) => {
  if (REF_TYPE === "branch") {
    import_node_assert.default.equal(await exec("git", ["rev-parse", "--verify", branch]), github.context.sha);
  }
};
var asserPreReleaseIdentifier = (branch) => {
  if (REF_TYPE === "branch") {
    import_node_assert.default.ok(/^[a-zA-Z0-9-]+$/.test(branch), "Branch name does not pass /^[a-zA-Z0-9-]+$/.");
  }
};
...
var getVersion = async () => {
  if (REF_TYPE === "tag") {
    ...
    if (lastGitTag === void 0) {
      return import_semver.default.parse(`0.1.0-${branch}+${COMMITISH}`, SEMVER_OPTIONS);
    } else {
      core.info(`Last tag: ${lastGitTag}`);
      const { major, minor, patch } = import_semver.default.parse(import_semver.default.clean(lastGitTag, SEMVER_OPTIONS), SEMVER_OPTIONS);
      return toSemver({
