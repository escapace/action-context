...
function reportStats(log$, opts) {
    ...
    if (!opts.isRecursive) {
        outputs.push(statsForCurrentPackage(log$.stats.pipe((0, operators_1.filter)((log) => log.prefix === opts.cwd)), {
            cmd: opts.cmd,
            width: opts.width,
        }));
    }
...
function statsForCurrentPackage(stats$, opts) {
    return stats$.pipe((0, operators_1.take)((opts.cmd === 'install' || opts.cmd === 'install-test' || opts.cmd === 'add' || opts.cmd === 'update' || opts.cmd === 'dlx') ? 2 : 1), (0, operators_1.reduce)((acc, log) => {
