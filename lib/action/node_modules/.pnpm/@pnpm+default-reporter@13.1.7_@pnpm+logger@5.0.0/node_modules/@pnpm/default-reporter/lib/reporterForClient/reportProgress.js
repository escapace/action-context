...
function reportProgress(log$, opts) {
    ...
    return getModulesInstallProgress$(log$.stage, log$.progress).pipe((0, operators_1.map)(opts.hideProgressPrefix
...
function throttledProgressOutput(opts, importingDone$, progress$) {
    ...
    return combinedProgress.pipe((0, operators_1.map)(opts.hideAddedPkgsProgress ? createStatusMessageWithoutAdded : createStatusMessage));
}
...
function createStatusMessageWithoutAdded([progress, importingDone]) {
    const msg = `Progress: resolved ${(0, outputConstants_1.hlValue)(progress.resolved.toString())}, reused ${(0, outputConstants_1.hlValue)(progress.reused.toString())}, downloaded ${(0, outputConstants_1.hlValue)(progress.fetched.toString())}`;
    if (importingDone) {
        return {
            done: true,
            fixed: false,
