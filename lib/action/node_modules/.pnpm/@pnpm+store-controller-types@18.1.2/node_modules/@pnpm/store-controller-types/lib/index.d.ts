import { type PkgResolutionId, type DirectoryResolution, type PreferredVersions, type Resolution, type WantedDependency, type WorkspacePackages } from '@pnpm/resolver-base';
...
import { type SupportedArchitectures, type DependencyManifest, type PackageManifest } from '@pnpm/types';
...
export type BundledManifest = Pick<DependencyManifest, 'bin' | 'bundledDependencies' | 'bundleDependencies' | 'dependencies' | 'directories' | 'engines' | 'name' | 'optionalDependencies' | 'os' | 'peerDependencies' | 'peerDependenciesMeta' | 'scripts' | 'version'>;
...
export type RequestPackageFunction = (wantedDependency: WantedDependency & {
    optional?: boolean;
}, options: RequestPackageOptions) => Promise<PackageResponse>;
